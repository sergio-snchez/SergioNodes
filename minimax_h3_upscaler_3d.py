# coding: utf-8
"""
SergioNodes Minimax H3 Latent Upscaler (3D) — clon local del nodo
"Minimax H3 Latent Upscaler (3D)" del pack Comfyui_Minimax_h3_latent_Upscaler.

Funciona de forma autónoma desde SergioNodes sin necesidad del custom node
original: mismo node_id/display name para mantener la compatibilidad de los
workflows, modelos en la carpeta modelos/latent_upscale_models.

Características del original (portadas intactas):
  - Temporal Chunking con Replicate Padding (evita OOM y flickering final)
  - Zero-Copy loading (evita picos de RAM) + soft_empty_cache de ComfyUI
  - Force-unload del modelo a CPU tras inferencia (liberar VRAM)
  - Alineado de la resolución a múltiplos de `align` (32 recomendado, evita
    light banding), con doble alineación pixel/latent
  - Backends cuda/rocm/cpu y precisiones fp32/fp16/bf16
  - 3 modos: scale by multiplier / target dimensions / megapixels
  - Autodetección de arquitectura del modelo (canales, bloques, temporal)
"""
...
class MinimaxH3LatentUpscaler3D(io.ComfyNode):
    """MiniMax H3 latent upscaler con Temporal Chunking y alineado a píxel físico."""
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Combo.Input("model_name", options=scan_models(), tooltip="Modelo de upscale Minimax H3."),

                io.DynamicCombo.Input(
                    "mode",
                    tooltip="Cómo se calcula el tamaño objetivo.",
                    options=[
                        io.DynamicCombo.Option(UpscaleMode.SCALE_BY, [
                            io.Float.Input("scale", default=2.0, min=1.0, max=4.0, step=0.05, tooltip="Factor de ampliación."),
                        ]),
                        io.DynamicCombo.Option(UpscaleMode.TARGET_DIMENSIONS, [
                            io.Int.Input("width", default=1280, min=64, max=8192, step=8, tooltip="Anchura objetivo en píxeles."),
                            io.Int.Input("height", default=704, min=64, max=8192, step=8, tooltip="Altura objetivo en píxeles.")
                        ]),
                        io.DynamicCombo.Option(UpscaleMode.MEGAPIXELS, [
                            io.Float.Input("megapixels", default=1.0, min=0.1, max=16.0, step=0.1, tooltip="Megapíxeles objetivo.")
                        ])
                    ],
                ),

                io.Int.Input("align", default=32, min=1, max=512, step=1,
                             tooltip="Alineado en píxel físico. 32 es lo recomendado estrictamente."),

                # ---- Comportamiento de inferencia ----
                io.Boolean.Input("enable_temporal_chunking", default=True,
                                 tooltip="Otorga chunking temporal para ahorrar VRAM en vídeos largos y arreglar el flickering del final."),
                io.Boolean.Input("force_unload", default=True,
                                 tooltip="Descarga el modelo a CPU tras la inferencia para liberar VRAM para los nodos siguientes. "
                                         "Desactívalo si ejecutas este nodo repetidamente y quieres evitar recargar."),
            ...
            outputs=[
                io.AnyType.Output("latent", tooltip="Latente ampliado."),
            ],
        )
    ...
    def execute(cls, latent: dict, model_name: str, mode: UpscaleConfig,
        ...
        # 2. Alineado bidireccional en píxel físico
